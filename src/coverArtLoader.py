...
def getCoverPath(uri, dim):
    if not getCoverPath.cache_path:
        getCoverPath.cache_path = BaseDirectory.save_cache_path(
...
def getErrorImage():
    if not getErrorImage.image:
        getErrorImage.image = Gtk.Image.new_from_icon_name(
...
def scaleToDimension(pixbuf, dim):
    ...
    return cropped.scale_simple(
        dim.width, dim.height, GdkPixbuf.InterpType.BILINEAR)
...
class PixbufCache:
    ...
    class PixbufCacheEntry:
        ...
        def get_scaled(self, uri, dim, urls):
            ...
            bigEnoughDims = [
                scale for scale in self.pixbufs_scaled.keys() if scale >= dim]
            if len(bigEnoughDims) > 0:
                self.pixbufs_scaled[dim] = scaleToDimension(
                    self.pixbufs_scaled[min(bigEnoughDims)], dim)
            else:
...
class CoverArtLoader:
    ...
    def asyncUpdateCover(self, updateMe, uri, urls,
        ...
        def __set_to_icon(icon_name, size):
            ...
            GLib.idle_add(
                ...
                function=__set_to_icon_helper)

        if urls is None:
            if uri == "Saved Tracks":
                __set_to_icon(
                    "emblem-favorite-symbolic.symbolic",
                    Gtk.IconSize.DIALOG)
            return
