...
class Login(Gtk.Bin):
    ...
    def __init__(self, onLoggedIn, **kwargs):
        ...
        check_login_with_automation = Gtk.CheckButton.new_with_label(
            "Login with browser automation !Automatically agrees to authorize this app! (experimental)")
        ...
        def onButtonPressed(button):
            def readInputsAndStartLoginThread():
                if check_login_with_automation.get_active():
                    threading.Thread(daemon=True, target=self.loginWithSelenium, args=(
                        username_input.get_text(), password_input.get_text())).start()
                else:
                    threading.Thread(
                        daemon=True, target=self.loginNormal, args=(
                            username_input.get_text(),)).start()
