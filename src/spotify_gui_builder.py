...
class SpotifyGuiBuilder:
    ...
    def __build_generic_entry(self, entry, image_responses,
        ...
        coverArt = self.cover_art_loader.get_loading_image()
        hbox.pack_start(coverArt, False, True, 5)
        self.cover_art_loader.async_update_cover(
            coverArt, urls=image_responses, uri=uri, dimensions=Dimensions(
        ...
        def _search_result_helper(
            ...
            if len(response['items']) != 0:
                ...
                not_found_label.set_markup(
                    'Could not find any ' + search_type +
    ...
    def async_load_playlists(self, playlists_list):
        ...
        def load_playlists():
            ...
            keep_going = True
            while keep_going:
                playlists_response = sp.get().current_user_playlists(
                    limit=page_size, offset=offset)
