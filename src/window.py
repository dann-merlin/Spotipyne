...
class SpotipyneWindow(Handy.ApplicationWindow):
    ...
    def initSimpleControls(self):
        ...
        self.SimpleControlsParent.pack_start(
            self.simpleControls, False, True, 0)
        self.simpleControls.set_reveal_child(False)

    def initLibraryOverview(self):
        self.libraryOverview = LibraryOverview(
            self.spGUI, self.BackButtonPlaylists)
        self.MainStack.add_titled(self.libraryOverview, 'Library', 'Library')
        self.MainStack.child_set_property(
            self.libraryOverview, 'icon-name',
    ...
    def initSearchOverview(self):
        ...
        self.MainStack.child_set_property(
            self.searchOverview, 'icon-name', 'edit-find-symbolic')

    def initBackButtons(self):
        self.BackButtonStack.child_set_property
        self.MainStack.bind_property(
            "visible-child-name",
            self.BackButtonStack,
            "visible-child-name")
    ...
    def onLoggedIn(self):
        ...
        self.HeaderbarSwitcher.bind_property(
            "title-visible",
            self.BottomSwitcher,
            "reveal",
